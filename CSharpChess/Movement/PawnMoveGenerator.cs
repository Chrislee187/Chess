...
namespace CSharpChess.Movement
{
    public class PawnMoveGenerator : MoveGeneratorBase
    {
        ...
        private static IEnumerable<Move> CalcTakes(CSharpChess.Board board, BoardLocation at)
        {
        ...
        private static IEnumerable<Move> CalcMoves(CSharpChess.Board board, BoardLocation at, Func<CSharpChess.Board, BoardLocation, Colours, bool> destinationCheck)
        {
        ...
        private static IEnumerable<BoardLocation> CalcNormalTakes(CSharpChess.Board board, BoardLocation at) 
            => TakeLocations(board, at).Where(p => board.CanTakeAt(p, board[at].Piece.Colour));

        private static IEnumerable<BoardLocation> TakeLocations(CSharpChess.Board board, BoardLocation at)
            => CreateTakeLocations(board, at, CalcTakeLocation);

        private static IEnumerable<BoardLocation> CalcEnPassants(CSharpChess.Board board, BoardLocation at) 
            => CreateEnPassantLocations(board, at)
                .Where(p => 
                    Pawns.EnPassantRules.Check(board, Move.Create(at, p)).All(rr => rr.Passed));

        private static IEnumerable<BoardLocation> CreateEnPassantLocations(CSharpChess.Board board, BoardLocation at) 
            => CreateTakeLocations(board, at, CalcEnPassantLocation);

        private static BoardLocation CalcTakeLocation(CSharpChess.Board board, BoardLocation at, Board.DirectionModifiers direction)
        {
        ...
        private static BoardLocation CalcEnPassantLocation(CSharpChess.Board board, BoardLocation at, Board.DirectionModifiers direction)
        {
            var vertical = Board.ForwardDirectionModifierFor(board[at].Piece);
        ...
        private static IEnumerable<BoardLocation> CreateTakeLocations(CSharpChess.Board board, BoardLocation at, 
            Func<CSharpChess.Board, BoardLocation, Board.DirectionModifiers, BoardLocation> positionCalculator)
        {
