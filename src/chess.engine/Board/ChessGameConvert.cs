...
namespace chess.engine.Board
{
    public static class ChessGameConvert
    {
        ...
        private static void CheckForCastleEligibility(ChessGame chessGameBoard, StringBuilder sb)
        {
            ...
            {
                bool CanCastle(MoveType castleSide)
                {
                    return kingMoves.Any(p => p.MoveType == castleSide);
                }

                sb.Append(CanCastle(MoveType.CastleQueenSide) ? "1" : "0");
                sb.Append(CanCastle(MoveType.CastleKingSide) ? "1" : "0");
            }
